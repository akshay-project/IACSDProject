...
const AuthenticationTeacher = require('../models/teacherAuthSchema')
...
app.post('/register', async (req, res) => {
    ...
    try {
        const userExist = await AuthenticationTeacher.findOne({ _id: _id });
        if (userExist) {
            return res.status(422).json({ err: "Id already axits" });
        } else if (copass != pass) {
            return res.status(422).json({ err: "Password Mismatch" });
        } else {
            const user = new AuthenticationTeacher({_id, name, email, pass, copass });
            //Conver to hash
...
app.post('/login', async (req, res) => {
    try {
        ...
        const userLogin = await AuthenticationTeacher.findOne({ _id: _id });
        if (userLogin) {
